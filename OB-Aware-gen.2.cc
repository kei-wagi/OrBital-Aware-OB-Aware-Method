 ...
 * Copyright (c) 2025 Keita Kashiwagi kashiwagi.keita415@mail.kyutech.jp
 *
 ...
 */
// ---------------------------------------------------------------- //
//            MODIFICATIONS FOR OB-AWARE ROUTING METHOD             //
// ---------------------------------------------------------------- //
/*
 * This file has been modified to implement the OrBital-Aware (OB-Aware)
 * Method, a routing protocol proposed in IEEE Access.
 *
 * Key Details:
 * - Implementation: Built as an extension of the AODV routing protocol
 * within the ns-3 simulator.
 * - Compatibility: Designed to work with the Gen. 1 LEO satellite
 * constellation.
 */
// ---------------------------------------------------------------- //
/*
 ...
 *
 * Original Authors: Elena Buchatskaia <borovkovaes@iitp.ru>
...
std::map<Ipv4Address, std::pair<double, int>> satelliteRreqTimeMap;
std::map<Ipv4Address, std::pair<double, int>> satelliteRrepTimeMap;
std::map<Ipv4Address, std::pair<double, double>> satelliteRttChangeMap;

  int obAwareFlag = 0;

  // Configuration for different satellite generations
  Ipv4Address dishAddress = "10.1.117.37";      // Dish (communication initiator)
  Ipv4Address groundStationAddress = "10.1.117.38";  // Ground station (destination)

  Ipv4Address satelliteBroadcastAddress = "10.1.255.255";
      ...
      if (!result || ((rt.GetFlag () != IN_SEARCH) && result))
        {
          ...
          SendRequest (header.GetDestination ()); //send first RREQ
        }
    }
}
...
RoutingProtocol::RouteInput (Ptr<const Packet> p, const Ipv4Header &header,
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb) //first send point??
{
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Create RREQ header
  RreqHeader rreqHeaderA;
  RreqHeader rreqHeaderB;
  rreqHeaderA.SetDst (dst);
  rreqHeaderB.SetDst (dst);
  ...
  if (m_routingTable.LookupRoute (dst, rt))
    {
      ...
      if (rt.GetValidSeqNo ())
        {
          rreqHeaderA.SetDstSeqno (rt.GetSeqNo ());
          rreqHeaderB.SetDstSeqno (rt.GetSeqNo ());
        }
      else
        {
          rreqHeaderA.SetUnknownSeqno (true);
          rreqHeaderB.SetUnknownSeqno (true);
        }
      rt.SetHop (ttl);
  ...
  if (m_gratuitousReply)
    {
      rreqHeaderA.SetGratuitousRrep (true);
      rreqHeaderB.SetGratuitousRrep (true);
    }
  if (m_destinationOnly)  {
      rreqHeaderA.SetDestinationOnly (true);
      rreqHeaderB.SetDestinationOnly (true);
    }

  m_seqNo++;
  rreqHeaderA.SetOriginSeqno (m_seqNo);
  m_seqNo++;
  rreqHeaderB.SetOriginSeqno (m_seqNo);
  m_requestId++;

  rreqHeaderA.SetId (m_requestId);
  m_requestId++;
  rreqHeaderB.SetId (m_requestId);
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      rreqHeaderA.SetOrigin (iface.GetLocal ());
      rreqHeaderB.SetOrigin (iface.GetLocal ());
      m_rreqIdCache.IsDuplicate (iface.GetLocal (), m_requestId);

      Ptr<Packet> packetA = Create<Packet> ();
      SocketIpTtlTag tag;
      tag.SetTtl (ttl);
      packetA->AddPacketTag (tag);
      packetA->AddHeader (rreqHeaderA);
      TypeHeader tHeader (AODVTYPE_RREQ);
      packetA->AddHeader (tHeader);
      // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
      Ptr<Packet> packetB = Create<Packet> ();
      tag.SetTtl (ttl);
      packetB->AddPacketTag (tag);
      packetB->AddHeader (rreqHeaderB);
      packetB->AddHeader (tHeader);
      // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
      ...
      NS_LOG_DEBUG ("Send RREQ with id " << rreqHeaderA.GetId () << " to socket");

      Simulator::Schedule ((MilliSeconds(0)), &RoutingProtocol::SendTo, this, socket, packetA, destination);
    
      Simulator::Schedule (MilliSeconds(1000), &RoutingProtocol::SendTo, this, socket, packetB, destination);

      Simulator::Schedule (MilliSeconds(1100), &RoutingProtocol::SendRequest2,this, "10.2.1.2");
    }
}

void
RoutingProtocol::SendRequest2 (Ipv4Address dst)
{

  obAwareFlag = 2;

  Ipv4Address maxDeltaSatellite = "102.102.102.102";
  Ipv4Address maxAlphaSatellite = "102.102.102.102";
  Ipv4Address maxBetaSatellite = "102.102.102.102";
  Ipv4Address fallbackSatellite = "102.102.102.102";
  Ipv4Address selectedSatellite;

  double maxDelta = -1;
  double maxAlphaOnly;
  double maxBetaOnly;

  for (const auto& [satelliteIp, rttChanges] : satelliteRttChangeMap) {
      double alphaValue = rttChanges.first;   // 1-hop RTT change (α)
      double betaValue = rttChanges.second;   // 2-hop RTT change (β)

      if (alphaValue > 0 && betaValue > 0) {
          // Calculate δ(i) = min(α, β) for both hop measurements available
          if (alphaValue >= betaValue){
            if(alphaValue > maxDelta){
              maxDelta = alphaValue;
              maxDeltaSatellite = satelliteIp;
            }
          }else if (alphaValue < betaValue)
          {
            if(betaValue > maxDelta){
              maxDelta = betaValue;
              maxDeltaSatellite = satelliteIp;
            }
          }
      } else if (alphaValue > 0 && betaValue < 0){
        // Only 1-hop measurement available
        if(alphaValue > maxAlphaOnly){
          maxAlphaOnly = alphaValue;
          maxAlphaSatellite = satelliteIp;
        }
      }else if (alphaValue < 0 && betaValue > 0)
      {
        // Only 2-hop measurement available
        if(betaValue > maxBetaOnly){
          maxBetaOnly = betaValue;
          maxBetaSatellite = satelliteIp;
        }
      }else if (alphaValue < 0 && betaValue < 0){
        // Fallback satellite with no measurements
        fallbackSatellite = satelliteIp;
      }
  }

  if (maxDeltaSatellite != "102.102.102.102"){
    selectedSatellite = maxDeltaSatellite;
  }else if (maxAlphaSatellite != "102.102.102.102")
  {
    selectedSatellite = maxAlphaSatellite;
  }else if (maxBetaSatellite != "102.102.102.102")
  {
    selectedSatellite = maxBetaSatellite;
  }else if (fallbackSatellite != "102.102.102.102")
  {
    selectedSatellite = fallbackSatellite;
  }
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
            m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
        {
          ...
          std::cerr<<"Selected satellite IP address: "<<selectedSatellite<<std::endl;

          socket->SendTo (packet, 0, InetSocketAddress (selectedSatellite, AODV_PORT));
          t++;
        }
}
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  Time t = Simulator::Now();
    ...
    if ( dishAddress != src && groundStationAddress != src && dishAddress == receiver && obAwareFlag != 2 ){

      RreqTime = Simulator::Now();
      double now = RreqTime.GetSeconds();
      double RTT_change;
      now = now - int(now);

      if(satelliteRreqTimeMap.find(src) == satelliteRreqTimeMap.end()){
        satelliteRreqTimeMap[src] = std::make_pair(now, 1);
        satelliteRreqTimeMap[src].second++;
      }
      else{
        // For second call, calculate time difference
        if (satelliteRreqTimeMap[src].second == 2) {
          double first_receive_time = satelliteRreqTimeMap[src].first;
          RTT_change = (first_receive_time - now) * 100000;
          satelliteRttChangeMap[src].first = RTT_change;
          std::cout<<"1 hop: "<<src<<" : "<<"RTT change (α): "<<RTT_change<<std::endl;
        }
      }
    }
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  Time t = Simulator::Now();
    ...
    if ( dishAddress != sender && groundStationAddress != sender && dishAddress == receiver && obAwareFlag != 2){
      RrepTime = Simulator::Now();
      RrepTime = RrepTime;
      double now = RrepTime.GetSeconds();
      double RTT_change;
      now = now - int(now);
      now = now - satelliteRreqTimeMap[sender].first;
      if(satelliteRrepTimeMap.find(sender) == satelliteRrepTimeMap.end()){
        satelliteRrepTimeMap[sender] = std::make_pair(now, 1);
        satelliteRrepTimeMap[sender].second++;
        return;

      }
      else{
        // For second call, calculate time difference (β-value for 2-hop RTT change) 
        if (satelliteRrepTimeMap[sender].second == 2) {
          double first_call_time = satelliteRrepTimeMap[sender].first;
          RTT_change = (first_call_time - now) * 100000;
          satelliteRttChangeMap[sender].second = RTT_change;          
          std::cout<<"2 hop: "<<sender<<" : "<<"RTT change (β): "<<RTT_change<<std::endl;
        }
      }
    }
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      if (!toDst.GetValidSeqNo ())
        {
          // m_routingTable.Update (newEntry); //check point3 nikai yobidash
        }
      // (ii)the Destination Sequence Number in the RREP is greater than the node's copy of the destination sequence number and the known value iss valid,
    ...
    {
      RoutingTableEntry AddEntry (/*device=*/ dev, /*dst=*/ dst, /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
                                          /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),/*hop=*/ hop,
                                          /*nextHop=*/ sender, /*lifeTime=*/ rrepHeader.GetLifeTime ());
      m_routingTable.AddRoute(newEntry);
    }
  ...
  NS_ASSERT (socket);
    ...
    if(obAwareFlag != 2){
    socket->SendTo (packet, 0, InetSocketAddress (satelliteBroadcastAddress, AODV_PORT));
  }else{
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), AODV_PORT));
  }
}
  ...
  if (toDst.GetFlag () == IN_SEARCH)
    {
      ...
      SendRequest (dst); //send second RREQ
    }
  else
    {
